...
class ResourceStreamer {
	...
	#fillQueue() {
		while ((this.#bytesQueued < this.#targetBytesQueued) &&
			...
			const use = Math.min(this.#targetBytesQueued - this.#bytesQueued, this.#bytesPerBlock)
			const slice = this.#resource.slice(this.#resource.position, this.#resource.position + use, false);
			this.#resource.position += slice.byteLength;
			this.#audio.enqueue(this.#stream, this.#audio.constructor.RawSamples, slice, 1, 0, slice.byteLength / this.#bytesPerSample);
			this.#audio.enqueue(this.#stream, this.#audio.constructor.Callback, slice.byteLength);
